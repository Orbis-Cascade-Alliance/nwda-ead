...
--><xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
                ...
                version="1.0"><!-- Set this variable to the server/folder path that points to the icon image file on your server.  
   ...
   <!-- ********************* <DSC> *********************** --><xsl:template name="dsc" match="dsc">
      <xsl:if test="@id"/>
      <fo:block font-size="24px" color="#676D38" margin-bottom="10px" margin-top="20px">
         <xsl:value-of select="$dsc_head"/>
      </fo:block>
      <fo:block>
         <xsl:choose><!-- if there are c02's apply normal templates --><xsl:when test="descendant::c02">
            ...
            <xsl:otherwise>
               ...
               <!-- if there are no c02's then all of the c01s are displayed as rows in a table, like an in-depth finding aid --><fo:table width="100%" table-layout="fixed">
                  <xsl:choose>
                     <xsl:when test="descendant::did/container[2]">
   ...
   <!-- ********************* </DSC> *********************** --><!-- ********************* <SERIES> *************************** --><xsl:template match="c01">
      <xsl:if test="@id"/>
      <xsl:for-each select=" *[@id] | did/*[@id]"/>
      <fo:block>
         <xsl:call-template name="dsc_table"/>
         <xsl:if test="//c02 or position()=last()"><!--<p class="top">
   ...
   <!-- ********************* END LABELS FOR TABLE ************************** --><!-- ********************* START c0xs *************************** --><xsl:template match="c02|c03|c04|c05|c06|c07|c08|c09|c10|c11|c12"><!-- this determines the number of containers (max of 2) so that when the template is called to display the text in the container
      ...
      <!-- ********* ROW FOR DISPLAYING CONTAINER TYPES ********* --><xsl:if test="did/container">
         <xsl:call-template name="container_row"/>
      </xsl:if>
      <!-- *********** ROW FOR DISPLAYING CONTAINER, CONTENT, AND DATE DATA **************--><!--all c0x level items are their own row; indentation created by css only--><fo:table-row width="100%"><!-- if there is only one container, the td is 170 pixels wide, otherwise 85 for two containers --><xsl:choose>
         ...
         <!-- this id is used for indentation; replaces nested tables --><!-- TOO BAD name() WORKS BETTER AND IDs MUST BE UNIQUE!. Excellent example of NIH.  - Enrico 10-18-07  --><!-- <xsl:variable name="c0x_content_indent">
				<xsl:choose>
			...
			</xsl:variable> --><xsl:variable select="count(../../preceding-sibling::*)+1" name="pppos"/>
         <xsl:variable select="count(../preceding-sibling::*)+1" name="ppos"/>
         <xsl:variable select="count(preceding-sibling::*)+1" name="cpos"/>
         <fo:table-cell border-bottom-color="#ddd" border-bottom-width="1px"
                        border-bottom-style="solid"
   ...
   <!-- APPLY TEMPLATES FOR UNITTITLE --><xsl:template match="unittitle">
      <xsl:apply-templates/>
   ...
   <!-- ********************* END c0xs *************************** --><!-- *** CONTAINER ROW ** --><xsl:template name="container_row"><!-- variables are created to grab container type data.
      ...
      <!-- if none of the container variables contains any data, the row will not be created --><xsl:if test="string($first_container) or string($second_container)">
         <fo:table-row width="100%">
            <xsl:choose><!-- for two containers --><xsl:when test="did/container[2]">
                  <fo:table-cell border-bottom-color="#ddd" border-bottom-width="1px"
                                 border-bottom-style="solid"
                                 padding="8px">
                     <fo:block>
   ...
   <!-- ******************** END CONVERT CONTAINER TYPE TO REGULAR TEXT ****************** --><xsl:template name="c0x_children"><!-- for displaying extent, physloc, etc.  this is brought over from the original mod.dsc --><!-- added note in addition to did/note for item 2F on revision specifications--><xsl:if test="string(did/origination | did/physdesc | did/physloc | did/note | arrangement | odd| scopecontent | acqinfo | custodhist | processinfo | note | bioghist | accessrestrict |    userestrict | index | altformavail)">
               ...
               <xsl:choose>
                  <xsl:when test="self::physdesc">
                     <fo:block>
                        ...
                        <!-- multiple extents contained in parantheses --><xsl:if test="string(extent[2])">
                           <xsl:text/>
                        ...
                        <!-- if genreform exists, insert a line break and then display genreforms separated by semicolons --><xsl:if test="genreform">
                           <fo:block/>
                        </xsl:if>
   ...
   <!-- kept from original mod.dsc --><xsl:template match="c01//did"><!-- c01 only --><xsl:choose><!-- original SY code
            ...
            <!-- 
                ...
                --><!-- <xsl:variable select="count(../preceding-sibling::*)+1" name="ppos"/> --><xsl:variable select="count(../preceding-sibling::c01)+1" name="ppos"/>
            <fo:block font-size="18px" color="#6b6b6b" margin-bottom="10px" margin-top="10px"><!-- what if no unitititle--><xsl:choose>
                  ...
                  <!-- SY Original Code
                     ...
                     <xsl:if test="$repCode='wau-ar' and unitid[@type='accession']"><!--  and ../c01[@otherlevel='accession'] --> Accession No.  </xsl:if>
            ...
            <!-- March 2015: Adding container display as per revision specification 7.1.2 --><xsl:if test="count(container) &gt; 0">
         ...
         <xsl:when test="$repCode='idu' or $repCode='ohy' or $repCode='orcsar' or $repCode='orcs' or $repCode='opvt' or $repCode='mtg' or $repCode='waps'"><!-- 2004-09-26 carlsonm mod to add display for <unitid> --><!-- Tracking # 4.10 Collins Land Company display --><xsl:if test="string(unitid)">
               <xsl:if test="unitid/@label">
                  <fo:inline color="#676d38" font-size="85%" text-decoration="none"
                             text-transform="capitalize">
                     <xsl:value-of select="unitid/@label"/>
            ...
            <!-- carlsonm 2004-09-26 not sure what the original intent was for this.  The <unitdate> element is not displaying in UMt Great Falls Breweries, Tracking #4.80 --><!--
				...
				--><!-- 2004-10-02 new mod for date so that empty elements will be ignored --><xsl:if test="string(unitdate) and string(unittitle)">, </xsl:if>
            <xsl:if test="string(unitdate)">
               <xsl:for-each select="unitdate">
                  <xsl:choose>
                     <xsl:when test="@type='bulk'">  (bulk <xsl:apply-templates/>) </xsl:when>
         ...
         <!-- carlsonm This is where the unittitle info is output when it is a c01 list only --><xsl:otherwise>
            ...
            <!-- END what if no unitititle--><!-- March 2015: Adding container display as per revision specification 7.1.2 --><xsl:if test="count(container) &gt; 0">
               <fo:block margin-bottom="10px">
                  <strong>Container(s): </strong>
                  <xsl:apply-templates select="container" mode="c01"/>
               </fo:block>
   ...
   <xsl:template match="daogrp">
      <xsl:choose><!-- First, check whether we are dealing with one or two <arc> elements --><xsl:when test="arc[2]">
            <xsl:if test="arc[2]/@show='new'"/>
            <xsl:for-each select="daoloc"><!-- This selects the <daoloc> element that matches the @label attribute from <daoloc> and the @to attribute
							from the second <arc> element --><xsl:if test="@label = following::arc[2]/@to">
         ...
         <!-- i.e. no second <arc> element --><xsl:otherwise>
            <xsl:choose>
               ...
               <xsl:when test="(arc[1][@show='replace'] or arc[1][@show='new']) and arc[1][@actuate='onrequest']">
                  <xsl:choose><!-- when a textual hyperlink is desired, i.e. <resource> element contains data --><xsl:when test="string(resource)">
                        <xsl:for-each select="daoloc">
                           <xsl:if test="@label = following::arc[1]/@to">
                              <xsl:attribute name="external-destination">
                                 <xsl:value-of select="@href"/>
                     ...
                     <xsl:otherwise><!-- if <resource> element is empty, produce an icon that can be used to traverse the link --><xsl:for-each select="daoloc">
                           <xsl:if test="@label = following::arc[1]/@to">
                              <xsl:attribute name="external-destination">
                                 <xsl:value-of select="@href"/>
                              </xsl:attribute>
