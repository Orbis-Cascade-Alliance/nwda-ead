	...
	<xsl:template name="toc">
		    ...
		    <fo:list-block provisional-distance-between-starts="15px" provisional-label-separation="5px">
               ...
               <fo:list-item-body start-indent="body-start()">
			      ...
			      <!-- ADMINISTRATIVE INFO -->
			<xsl:if test="string(arrangement) or string(custodhist) or string(acqinfo)       or string(processinfo) or string(accruals) or      string(separatedmaterial) or string(originalsloc)     or string(bibliography) or string(otherfindaid) or string(relatedmaterial) or      string(index)">
				        <fo:list-item>
               <fo:list-item-label end-indent="label-end()">
                  <fo:block/>
               </fo:list-item-label>
