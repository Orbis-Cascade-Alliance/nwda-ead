...
--><xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
   ...
   <xsl:variable name="hasCHOs">
      <xsl:if test="$harvester-active = 'true'"><!-- If there is an ARK in eadid/@identifier, at least one dao with a 'harvest-all' @role, and CHOs have been harvested, then CHOs is true --><xsl:if test="string(//*[local-name()='eadid']/@identifier) and descendant::*[local-name()='dao'][@*[local-name()='role']='harvest-all' and string(@*[local-name()='href'])] and document(concat('http://harvester.orbiscascade.org/apis/ask?ark=ark:/', //*[local-name()='eadid']/@identifier))//response='true'">
   ...
   <!-- if 'true', will group controlled access points: all persnames together, for example--><xsl:variable name="groupControlaccess">false</xsl:variable>
   ...
   <!--actual--><xsl:param name="arrangement_head">Arrangement</xsl:param>
   ...
   <!-- carlsonm mod 2004-07-09 adding a param for the 'Historical Background' heading --><xsl:param name="odd_head_histbck">Historical Background</xsl:param>
   ...
   <!-- ********************* </SECTION HEADS> *********************** --><!-- ********************* <LABELS> *********************** --><!-- March 2015 note: if NWDA migrates to an XSLT 2.0 processor, recommending normalization based on local-name() in an XSL function --><!--Section heads and other labels will default to these values if there is no head or labal present in the EAD original--><xsl:param name="label_style">asis</xsl:param>
   <!-- refers to css sheet. current valid values are caps, smcaps, titlecase, asis--><!--misc--><xsl:param name="sponsor_label">Sponsor</xsl:param>
   <xsl:param name="creation_label">Created</xsl:param>
   <xsl:param name="revision_label">Revisions</xsl:param>
   <!--did children--><xsl:param name="abstract_label">Summary</xsl:param>
   ...
   <!--"Collector" is also allowed--><xsl:param name="physdesc_label">Quantity</xsl:param>
   ...
   <!--did siblings--><xsl:param name="accessrestrict_label">Access Restrictions</xsl:param>
