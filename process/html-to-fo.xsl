...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:vcard="http://www.w3.org/2006/vcard/ns#" xmlns:xsd="http://www.w3.org/2001/XMLSchema#"
	xmlns:fo="http://www.w3.org/1999/XSL/Format" xmlns:nwda="https://github.com/ewg118/nwda-editor#" xmlns:arch="http://purl.org/archival/vocab/arch#" exclude-result-prefixes="arch nwda xsd vcard" version="1.0">
	...
	<xsl:template match="xsl:template[@name='html_base']">
		<xsl:element name="xsl:template">
			...
			<fo:root font-size="14px" color="#6b6b6b" font-family="georgia, 'times new roman', times, serif">
				<!--  -->
				...
				<fo:page-sequence master-reference="content">
					...
					<fo:flow flow-name="body">
						<fo:block font-size="36px" color="#676D38">
							<xsl:element name="xsl:value-of" namespace="http://www.w3.org/1999/XSL/Transform">
								<xsl:attribute name="select">normalize-space(archdesc/did/unittitle)</xsl:attribute>
							</xsl:element>
							<xsl:element name="xsl:if" namespace="http://www.w3.org/1999/XSL/Transform">
								...
								<xsl:element name="xsl:value-of" namespace="http://www.w3.org/1999/XSL/Transform">
									<xsl:attribute name="select">archdesc/did/unitdate</xsl:attribute>
								</xsl:element>
							</xsl:element>
						</fo:block>
						<fo:block>
	...
	<xsl:template match="span[string-length(normalize-space(.)) = 0]"/>
	...
	<xsl:template match="td">
		<fo:table-cell border-bottom-color="#ddd" border-bottom-width="1px" border-bottom-style="solid">
			<xsl:if test="@colspan">
	...
	<xsl:template match="th">
		<fo:table-cell border-bottom-color="#ddd" border-bottom-width="2px" border-bottom-style="solid">
			<xsl:if test="@colspan">
				<xsl:attribute name="number-columns-spanned">
					<xsl:value-of select="@colspan"/>
				</xsl:attribute>
